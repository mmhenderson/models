...
dataset_dir_root = '/usr/local/serenceslab/maggie/biasCNN/datasets/testing/'
...
def _get_filenames_and_classes(image_dir, noise_level):
  ...
  for ii in np.arange(0,np.size(model_labels)):
    subfolder = "SF_%.2f_noise%.2f" % (sf_vals[int(sflist[ii])], noise_level)
...
def main(argv):
  ...
  for nn in range(nNoiseLevels):
    ...
    #%% get the information for ALL my images (all categories, exemplars, rotations)
      ...
      all_filenames, all_labels, class_names = _get_filenames_and_classes(image_dir, noise_levels[nn])
